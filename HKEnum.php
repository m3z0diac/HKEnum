...
function get_url()
{
	# get the URL of the target web application
...
{
	# admin panel founder main function
...
function linksEnum()
{
	...
	for( $i = 0; $i <= count($myVar); $i++ ) {
		if (@substr($myVar[$i], 0, 4) == "href") {
			...
			if (count($link) <= 3) {
				@$sub = $link[2];
				array_push($result, rtrim($sub, "&#039; &quot &gt;&lt; span"));
			}
			elseif(count($link) > 3) {
				if ($link[3][0] != "%") {
					$sub = $link[2] . "/" .$link[3];
					array_push($result, rtrim($sub, "&#039; &quot"));
				}
			}
...
<body>
	<div>
		...
		<form action="HKEnum.php">
			...
			<label><input type="radio" name="option" value="cms">CMS Recon</label><br>
			<label><input type="radio" name="option" value="all">Scan All fields</label><br>
	...
	<?php
		...
		elseif (get_option() == "cms") {
		 	cmsRecon();
		}
		elseif (get_option() == "all") {
			adminPanel($admins);
			cmsRecon();
			linksEnum();
		}
